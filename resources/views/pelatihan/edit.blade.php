...
<div class="col-md-6">
    ...
    <div class="card card-success">
        ...
        <!-- /.card-header -->
        <!-- form start -->
        <form action="{{ route('pelatihan.update', $pelatihan->id) }}" action="{{ route('pelatihan.store') }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <div class="form-check">
                        <input
                            ...
                            name="status"
                            value="aktif"
                            {{ $pelatihan->status == 'aktif' ? 'checked' : '' }}>
                        <label class="form-check-label" for="status_aktif">Aktif</label>
                    </div>
                    <div class="form-check">
                        <input
                            ...
                            name="status"
                            value="tidak aktif"
                            {{ $pelatihan->status == 'tidak aktif' ? 'checked' : '' }}>
                        <label class="form-check-label" for="status_tidak_aktif">Tidak Aktif</label>
                    </div>
                </div>
            </div>
