        ...
        <div class="card-body">
            <table id="example1" class="table table-bordered table-striped ">
                ...
                <tbody>
                    ...
                    @foreach($datapelatihan as $dt)
                    <tr>
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ $dt->user->name }}</td>
                        <td>{{ $dt->pelatihan->nama_pelatihan}}</td>
                        <td>{{ $dt->nama }}</td>
                        <td>{{ $dt->nik }}</td>
                        <td>{{ $dt->alamat }}</td>
                        <td>{{$dt->no_hp}} </td>
                        <td>
                            <div class="btn-group">
                                <form action="{{ route('datapelatihan.destroy',$dt->id)}}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a type="button" class="btn btn-warning" href="{{ route('datapelatihan.edit',$dt->id) }}">
